    ...
    const candidates = await page.evaluate(() => {
      ...
      allElements.forEach((el) => {
        ...
        elements.push({
          ...
          parentClasses: el.parentElement ? Array.from(el.parentElement.classList).slice(0, 5) : []
        });
      });
      return elements;
    });
...
function generateLocators(action) {
  ...
  if (action.scrollX !== void 0)
    fp.scrollX = action.scrollX;
  if (action.scrollY !== void 0)
    fp.scrollY = action.scrollY;
  return {
    testId: fp.testId,
...
var SelfHealingResolver = class {
  ...
  extractSelectorFromStrategy(strategy, locators) {
...
`;
  ...
  generatePythonAction(action, indent) {
    ...
    switch (action.action_type) {
      ...
      case "select":
        return selector ? `${indent}${selector}.select_option("${this.escapeString(locators.fingerprint?.value || "")}")` : null;
      case "submit":
        return selector ? `${indent}${selector}.press("Enter")` : null;
      case "scroll": {
        const sx = locators.fingerprint?.scrollX ?? 0;
        const sy = locators.fingerprint?.scrollY ?? 0;
  ...
  generateJavaAction(action, indent) {
    ...
    switch (action.action_type) {
      ...
      case "input":
      case "change":
        return javaLocator ? `${indent}${javaLocator}.fill("${this.escapeString(locators.fingerprint?.value || "")}");` : null;
      case "keydown":
        return `${indent}page.keyboard().press("${this.escapeString(locators.fingerprint?.value || "Enter")}");`;
      case "select":
        return javaLocator ? `${indent}${javaLocator}.selectOption("${this.escapeString(locators.fingerprint?.value || "")}");` : null;
      case "submit":
        return javaLocator ? `${indent}${javaLocator}.press("Enter");` : null;
      case "scroll": {
        const sx = locators.fingerprint?.scrollX ?? 0;
        const sy = locators.fingerprint?.scrollY ?? 0;
        return `${indent}page.evaluate("window.scrollTo(${sx}, ${sy})");`;
  ...
  generateCSharpAction(action, indent) {
    ...
    switch (action.action_type) {
      ...
      case "input":
      case "change":
        return csLocator ? `${indent}await ${csLocator}.FillAsync("${this.escapeString(locators.fingerprint?.value || "")}");` : null;
      case "keydown":
        return `${indent}await page.Keyboard.PressAsync("${this.escapeString(locators.fingerprint?.value || "Enter")}");`;
      case "select":
        return csLocator ? `${indent}await ${csLocator}.SelectOptionAsync("${this.escapeString(locators.fingerprint?.value || "")}");` : null;
      case "submit":
        return csLocator ? `${indent}await ${csLocator}.PressAsync("Enter");` : null;
      case "scroll": {
        const sx = locators.fingerprint?.scrollX ?? 0;
        const sy = locators.fingerprint?.scrollY ?? 0;
  ...
  getBestLocatorCode(locators) {
    ...
    if (locators.altText)
      return `page.getByAltText('${this.escapeString(locators.altText)}')`;
    if (locators.title)
      return `page.getByTitle('${this.escapeString(locators.title)}')`;
    if (locators.css)
  ...
  getBestPythonLocator(locators) {
    ...
    if (locators.altText)
      return `page.get_by_alt_text("${this.escapeString(locators.altText)}")`;
    if (locators.title)
      return `page.get_by_title("${this.escapeString(locators.title)}")`;
    if (locators.css)
      return `page.locator("${this.escapeString(locators.css)}")`;
    return null;
  }
  getBestJavaLocator(locators, indent) {
    if (locators.testId)
      return `page.getByTestId("${this.escapeString(locators.testId)}")`;
    if (locators.role)
      return `page.getByRole(AriaRole.${locators.role.role.toUpperCase()}, new Page.GetByRoleOptions().setName("${this.escapeString(locators.role.name)}"))`;
    if (locators.label)
      return `page.getByLabel("${this.escapeString(locators.label)}")`;
    if (locators.text)
      return `page.getByText("${this.escapeString(locators.text)}")`;
  ...
  getBestCSharpLocator(locators) {
    if (locators.testId)
      return `page.GetByTestId("${this.escapeString(locators.testId)}")`;
    if (locators.role)
      return `page.GetByRole(AriaRole.${this.toPascalCase(locators.role.role)}, new() { Name = "${this.escapeString(locators.role.name)}" })`;
    if (locators.label)
      return `page.GetByLabel("${this.escapeString(locators.label)}")`;
    if (locators.text)
      return `page.GetByText("${this.escapeString(locators.text)}")`;
